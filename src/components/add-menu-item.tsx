...
export function AddMenuItem({ restaurantId, onSkip, isDialog = false }: AddMenuItemProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const timeRanges = availableTimes.map(time => {
        switch (time) {
          case 'morning':
  ...
  // Wrap the content in a dialog if isDialog is true
  const content = (
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          <h3 className="text-lg font-medium">
            {isDialog ? 'Add Menu Item' : 'Add Your First Menu Item'}
          </h3>
          <p className="text-sm text-muted-foreground">
            {isDialog 
              ? 'Add a new item to your menu.'
              : 'Get started by adding your first menu item. You can add more items later.'}
          </p>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-sm">
            <span>{currentStep === 'basic' ? 'Basic Information' : 'Additional Details'}</span>
            <span>Step {currentStep === 'basic' ? '1' : '2'} of 2</span>
          </div>
          <Progress value={currentStep === 'basic' ? 50 : 100} />
        </div>
            ...
            <Button type="submit" disabled={isLoading}>
              {isLoading ? (
                <>
                  ...
                  {currentStep === 'basic' ? 'Next' : 'Adding...'}
                </>
              ) : (
                currentStep === 'basic' ? 'Next' : 'Add Menu Item'
              )}
            </Button>
          </div>
        </form>
